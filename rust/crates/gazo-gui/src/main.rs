...
// "ICU4X data error: No segmentation model for language: ja" を大量に出す（表示は正常）。
//
// icu_provider は logging フィーチャ無効だと内部 `log::warn!` が `std::eprintln!` に化けて
// 無条件で stderr 出力する。Cargo.toml で logging を有効化し本物の log 経由にしたうえで、
// ここで `log::max_level` を Off に落として実行時に遮断する。Slint バックエンドが
// ロガー初期化時に max_level を Warn 以上へ戻すため、バックエンド初期化後にも当て直す。
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // AppWindow::new() でバックエンド（とそのロガー）が初期化され max_level を戻すため、当て直す。
